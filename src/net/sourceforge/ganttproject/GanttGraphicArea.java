...
public class GanttGraphicArea extends JPanel {
            ...
            if (param.type == 0) {
              ...
              try {
                setCursor(toolkit.createCustomCursor(
                    toolkit.getImage(getClass().getClassLoader().getResource("icons/cursorpercent.gif")),
      ...
      public void mouseDragged(MouseEvent e) {
        ...
        if (mouseButton == MouseEvent.BUTTON1) {
          if (moveTask >= 0) {
             ...
             * if(taskDuration>=1)taskToMove.setStart(storeTaskStart.newAdd((int)
             * addTaskLength));
  ...
  /**
   * Detect if the position of the mouse is on a special place (return -1 if
   * nothing or the numer of the task
   */
  ...
  public void changeDate2(GanttCalendar gc) {
    ...
    switch (zoomValue) {
      ...
      case THREE_YEAR:
        ...
        break;
      // default:System.out.println("Gantt : Should never append -->
      // zoomValue="+zoomValue);
    }
  }
  ...
  public boolean isVisible(Task thetask) {
    ...
    while (father != null) {
      // if (!expand.contains(new
      // Integer(((Task)(father.getUserObject())).getTaskID()))) {
      ...
      case FOUR_MONTH:
        ...
        for (int i = 0; i < 4; i++) {
          ...
          g.drawString(dm.substring(0,
              (sizex > 480) ? (dm.length() < 3 ? dm.length() : 3) : 1) +
              "  " + new Integer(cal.getYear()).toString().substring(2, 4),
              (int) posX + 2 /* i*sizex/3+2 */, headery / 2 - 5);
          posX += ((float) cal.getNumberOfDay() * fgra);
          cal.goNextMonth();
        }
      ...
      switch (zoomValue) {
        case ONE_WEEK:
          g.drawString(sDay.substring(0,
              (sizex > 300) ? (sDay.length() < 3 ? sDay.length() : 3) : 1) + " " + tmpdate.getDate(),
        ...
        case ONE_YEAR:
          g.drawString(sMonth.substring(0,
              (sizex > 300) ? (sMonth.length() < 3 ? sMonth.length() : 3) : 1), (int) (fgra * i) + 2,
              headery - 7);
          g.setColor(arrayColor[4]);
          g.drawLine((int) (fgra * i), headery / 2, (int) (fgra * i),
              headery - 1);
  ...
  public void paintATaskChild(Graphics g, int x1, int x2, int y, Task task) {
    ...
    // if(draw3dBorders)
    {
      ...
      // - for the rendering, it's only beautiful with the default color, but not with
      // another one
  ...
  public BufferedImage getChart(GanttExportSettings settings) {
    ...
    sizeTOC += 50; // for the indentation of the tasks, but I fiw 50 pixel correcpond to 7 sub-taks
                   // indentation
  ...
  public void printProject(GanttExportSettings settings) {
    // For printing the project, begin to create temporary BufferedImage for the
    // entire graphics
  ...
  /**
   * Resizes the area to fit the whole project. Height is set according to the
   * number of tasks.
